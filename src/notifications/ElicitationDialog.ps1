...
.DESCRIPTION
    Called when Claude Code presents a dialog requiring user interaction.
    Uses the Reminder scenario to persist in Action Center.
#>
param(
    [hashtable]$HookData,
    [hashtable]$Config,
...
$message = if ($HookData.message) { $HookData.message } else { "Claude is waiting for your response." }
...
$toastParams = @{
    ...
    UniqueIdentifier = "claude-elicitation-$sessionId"
}
