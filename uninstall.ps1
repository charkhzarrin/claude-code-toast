<#
.SYNOPSIS
    Removes Claude Code Toast notifications.
.DESCRIPTION
    1. Removes Stop hook from Claude Code settings.json
...
if (Test-Path $settingsPath) {
    try {
            ...
            if ($settings.hooks.PSObject.Properties.Name -contains $eventName) {
                $filtered = @($settings.hooks.$eventName | Where-Object {
                    ...
                    foreach ($h in @($_.hooks)) {
                        if ($h.command -like "*ClaudeCodeToast*" -or $h.command -like "*hook.ps1*" -or $h.command -like "*Send-ClaudeToast*") {
                            $keep = $false; break
                        }
                    }
                    $keep
        ...
        if ($changed) {
            ...
            Write-Host "  Hook removed - OK" -ForegroundColor Green
        } else {
            Write-Host "  No hook found - skipping" -ForegroundColor Yellow
        }
    } catch {
...
if (Get-Module -ListAvailable -Name BurntToast) {
    $response = Read-Host "  Remove BurntToast? Other scripts may use it. (y/N)"
